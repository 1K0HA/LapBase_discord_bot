...
if exist "pyproject.toml" (
  for /f "tokens=3" %%V in ('findstr /b /c:"version = " "pyproject.toml" 2^>nul') do set "APP_VERSION=%%~V"
)
...
title LapBase v!APP_VERSION! - Публикация в GitHub
...
:header
echo ============================================================
echo          LapBase v!APP_VERSION! - Публикация в GitHub
echo ============================================================
echo Кодировка:
echo   UTF-8
echo Проект:
...
:preflight_failed
echo.
echo [БЛОКИРОВКА] Предварительная проверка не пройдена. Исправьте указанные проблемы и запустите снова.
exit /b !RC!
...
for /f "delims=" %%V in ('git --version 2^>nul') do echo [ГОТОВО] %%V
goto :pf_git_done
...
for /f "tokens=1,2,3" %%A in ('gh --version 2^>nul ^| findstr /b /c:"gh version"') do echo [ГОТОВО] GitHub CLI %%C
goto :pf_gh_done
...
:pf_no_origin
echo [ПРЕДУПРЕЖДЕНИЕ] Удалённый origin не настроен.
...
rem Сначала проверяем проектную политику напрямую по .gitignore.
rem Это работает до git init и сохраняет preflight в режиме только чтения.
findstr /x /l /c:"!IGNORE_RULE!" ".gitignore" >nul 2>nul
if errorlevel 1 goto :cir_missing_rule

rem Если путь ещё не существует, достаточно наличия самого правила.
if not exist "!IGNORE_PATH!" goto :cir_rule_ok

rem До git init у git check-ignore нет контекста репозитория.
if not "!IS_REPO!"=="1" goto :cir_rule_ok

rem После создания репозитория дополнительно проверяем фактическое решение Git.
...
:setup_repo_exists
echo [ГОТОВО] Локальный репозиторий уже существует.
...
echo Будет добавлен удалённый репозиторий:
...
echo 1. Приватный (private)
echo 2. Публичный (public)
...
echo   удалённый репозиторий: origin
set "ANSWER="
...
if not exist "uv.lock" goto :push_no_lock
...
echo   сообщение commit: !COMMIT_MSG!
...
:push_no_origin
echo [ОШИБКА] Удалённый origin отсутствует.
exit /b 13

:push_no_lock
echo [БЛОКИРОВКА] Файл uv.lock отсутствует.
echo Что можно сделать: запустите start.bat --prepare-only --dev, затем добавьте созданный uv.lock в Git.
exit /b 13
...
echo         fix: исправить переносы строк Telegram
exit /b 31
...
:release_no_repo
echo [ОШИБКА] Локальный репозиторий отсутствует.
exit /b 13
