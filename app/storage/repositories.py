...
from app.storage.models import POST_RECORD_COLUMNS, POST_RECORD_SELECT, PostRecord
...
def _record(row) -> PostRecord | None:
    if row is None:
        return None
    data = dict(row)
    # Фильтруем только контрактные поля: будущая дополнительная колонка БД
    # не должна ломать уже запущенную версию Repository.
    return PostRecord(**{name: data[name] for name in POST_RECORD_COLUMNS})
...
class Repository:
    ...
    async def enqueue_retry(self, message_id: int) -> bool:
        row = await self.pool.fetchrow(
            ...
            UPDATE posts
               SET status='queued',
                   pending_action=CASE
                       WHEN telegram_message_id IS NULL THEN 'publish'
                       ELSE 'edit'
                   END,
        ...
        row = await self.pool.fetchrow(
            ...
            UPDATE posts
               SET status='queued', pending_action='republish', retry_count=0,
                   ...
                   telegram_media_message_ids=CASE
                       WHEN status='deleted' THEN '{}'::BIGINT[]
                       ELSE telegram_media_message_ids
                   END,
    ...
    async def get_post(self, message_id: int) -> PostRecord | None:
        query = f"SELECT {POST_RECORD_SELECT} FROM posts WHERE discord_message_id=$1"
        return _record(await self.pool.fetchrow(query, message_id))

    async def get_oldest_pending(self) -> PostRecord | None:
        query = (
            f"SELECT {POST_RECORD_SELECT} FROM posts "
            "WHERE status = ANY($1::text[]) "
            "ORDER BY queued_at ASC, discord_message_id ASC LIMIT 1"
        )
        return _record(await self.pool.fetchrow(query, list(PENDING_STATUSES)))
    ...
    async def mark_published(self, message_id: int, telegram_message_id: int) -> None:
        await self.pool.execute(
            """
            UPDATE posts SET status='published', pending_action='publish',
                telegram_message_id=$2, telegram_media_message_ids='{}'::BIGINT[],
    ...
    async def clear_database(self) -> dict[str, int]:
        """Удаляет данные LapBase, сохраняя схему базы данных."""
