...
class DiscordSourceClient(discord.Client):
    ...
    async def on_raw_message_edit(self, payload: discord.RawMessageUpdateEvent) -> None:
        ...
        if await self.repo.enqueue_edit(payload.message_id, payload.channel_id):
            ...
            self.wake_worker()
            return
        # Редкая гонка: edit может прийти раньше, чем create успел сохраниться в БД.
        try:
            channel = await self.get_text_channel(payload.channel_id)
            async with asyncio.timeout(self.config.discord_api_timeout_seconds):
                message = await channel.fetch_message(payload.message_id)
            if await self.repo.enqueue_new(message.id, payload.channel_id, message.created_at):
                self.wake_worker()
        except Exception:
            logger.exception("Не удалось восстановить неизвестное отредактированное сообщение %s", payload.message_id)
    ...
    async def get_text_channel(self, channel_id: int):
        ...
        if not isinstance(channel, (discord.TextChannel, discord.Thread)):
            raise RuntimeError(f"Discord channel {channel_id} не является text channel/thread")
        return channel
    ...
    async def sync_history(self) -> int:
        ...
        for channel_id in self.config.discord_channel_ids:
            try:
                channel = await self.get_text_channel(channel_id)
                history_timeout = max(self.config.discord_api_timeout_seconds * 4, 60)
                ...
                logger.exception("Не удалось синхронизировать Discord channel %s", channel_id)
